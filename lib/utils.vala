...
namespace Utils {
    ...
    public string get_root_path(string file) {
        return GLib.Path.build_path(Path.DIR_SEPARATOR_S, GLib.Path.get_dirname((string) project_path()), file);
    }
    ...
    public string get_config_dir() {
		return GLib.Path.build_path(Path.DIR_SEPARATOR_S, Environment.get_user_config_dir(), "deepin", "deepin-designer");
	}

    public string get_config_file_path(string config_name) {
		return GLib.Path.build_path(Path.DIR_SEPARATOR_S, Environment.get_user_config_dir(), "deepin", "deepin-designer", config_name);
	}
   ...
   public void get_pointer_position(out int x, out int y) {
    ...
    public void show_manual() {
        if (Utils.is_command_exist("dman")) {
            try {
                GLib.AppInfo appinfo = GLib.AppInfo.create_from_commandline("dman deepin-designer", null, GLib.AppInfoCreateFlags.NONE);
